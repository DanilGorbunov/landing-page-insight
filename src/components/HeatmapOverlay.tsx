...
/**
 * Full-bleed attention heatmap layer (visual only). Details use unified AI label dots on the screenshot.
 */
export function HeatmapOverlay({ zones }: { zones: AttentionZone[] }) {
  const sorted = [...zones].filter(Boolean).sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  return (
    <div className="pointer-events-none absolute left-0 top-0 h-full w-full overflow-visible" style={{ zIndex: 10 }}>
      {sorted.map((zone, i) => (
        <div
          key={`${zone.order}-${i}`}
          className="absolute overflow-visible transition-opacity duration-300 ease-out"
          style={{
